...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  _.each(txp.inputs, function(i) {
	console.log("############### i.path: " + JSON.stringify(i.path) );
...
API.prototype._signTxp = function(txp, password) {
  ...
  console.log("############## API.prototype._signTxp derived: " + JSON.stringify(derived) );
...
API.prototype.createAddress = function(opts, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  console.log("$$$$$$$$$$$$$$$$$$$ createAddress opts: " + JSON.stringify(opts) );
  var self = this;
  ...
  self._doPostRequest('/v3/addresses/', opts, function(err, address) {
    if (err) return cb(err);
    console.log("$$$$$$$$$$$$$$$$$$$ createAddress address: " + address )
...
API.prototype.signTxProposal = function(txp, password, cb) {
  console.log("######################## signTxProposal: v1/txproposals txp: " + JSON.stringify(txp) );
  console.log("######################## signTxProposal: v1/txproposals password: " + JSON.stringify(password) );
  $.checkState(this.credentials && this.credentials.isComplete());
...
API.prototype.broadcastTxProposal = function(txp, cb) {
  $.checkState(this.credentials && this.credentials.isComplete());
  log.debug("################ broadcastTxProposal : v1/txproposals/txp.id/broadcast");
  var self = this;
